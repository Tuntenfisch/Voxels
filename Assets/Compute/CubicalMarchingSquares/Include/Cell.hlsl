...
bool IsOutsideCell(float3 cellSpacePosition, float epsilon = 0.0f)
{
    float3 min = (float3) (cellCorners[0] - epsilon);
    float3 max = (float3) (cellCorners[6] + epsilon);
    
    return any(cellSpacePosition < min || cellSpacePosition > max);
}

float3 ClampToCell(float3 cellSpacePosition, float epsilon = 0.0f)
{
    float3 min = (float3) (cellCorners[0] - epsilon);
    float3 max = (float3) (cellCorners[6] + epsilon);
    
    return clamp(cellSpacePosition, min, max);
}
